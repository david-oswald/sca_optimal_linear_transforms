...
%%%% band-limited noise powers (pow_band_noise) for randomization over rand_cycle_count cycles
close all;
clear all;
...
for pow_band_noise = [10 15 20]
    ...
    % save figures
    saveas(corr_plot_global, sprintf('%s/%d_profiling_jitter_noise_%f.fig', project_path, trace_count, pow_band_noise)) 
    saveas(a_plot, sprintf('%s/%d_profiling_jitter_noise_%f_freq.fig', project_path, trace_count, pow_band_noise)) 
    saveas(trace_plot, sprintf('%s/%d_profiling_jitter_noise_%f_traces.fig', project_path, trace_count, pow_band_noise)) 
    ...
    saveas(corr_plot_global, sprintf('%s/%d_attack_jitter_noise_%f.fig', project_path, trace_count, pow_band_noise)) 
end
