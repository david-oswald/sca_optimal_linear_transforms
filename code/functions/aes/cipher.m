function ciphertext = cipher (plaintext, w, s_box, poly_mat, rounds, vargin)
...
%ciphertext.initial = zeros(10, 16);
% Loop over 9 rounds
for i_round = 1 : 9
    
    if i_round == rounds
        ciphertext.initial = reshape (state, 1, 16);
    end
    ...
    if i_round == rounds
        ciphertext.sub_bytes = reshape (state, 1, 16);
    end
    ...
    if i_round == rounds
        ciphertext.shift_rows = reshape (state, 1, 16);
    end
    ...
    [state red_count] = mix_columns (state, poly_mat);
    
    if i_round == rounds
        ciphertext.mix_columns = reshape (state, 1, 16);
        ciphertext.red_count = red_count;
    end
    ...
    if i_round == rounds
        ciphertext.round_key = reshape(round_key, 1, 16);
    end
    ...
    if i_round == rounds
        ciphertext.add_round_key = reshape (state, 1, 16);
        return;
    end
...
ciphertext.result = reshape (state, 1, 16);