function plaintext = inv_cipher (ciphertext, w, inv_s_box, inv_poly_mat, rounds, vargin)
...
for i_round = 9 : -1 : 1
    ...
    if i_round == rounds
        plaintext.initial = reshape (state, 1, 16);
    end
    ...
    if i_round == rounds
        plaintext.inv_sr = reshape (state, 1, 16);
    end
    ...
    if i_round == rounds
        plaintext.inv_sb = reshape (state, 1, 16);
    end
    ...
    if i_round == rounds
        plaintext.add_round_key = reshape (state, 1, 16);
    end
    ...
    if i_round == rounds
        plaintext.mix_col = reshape (state, 1, 16);
    end
end
...
plaintext.result = reshape (state, 1, 16);
